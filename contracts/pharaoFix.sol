...
contract PharaohFarmPoTFull is Ownable, ReentrancyGuard{
    ...
    IERC20 CAFToken = IERC20(0x2c21f3B09fFf5f0a98EaeB6DE9fA8E6011f1E9c7);
    struct User {
        address userAddress;
        ...
        for (uint256 i = 0; i < 6; i++) {
            if (uplineUserAddress != address(0) && uplineUserAddress != firstAddress) {
                if (currentDistributionLevel < 7 ) {
                    //NOTE: cache to INT in order to avoid revert in line 1120
                    int rew = int(((amount *
...
emit Reffered(msg.sender,uplineUserAddress,uint(rew));
    ...
    function reinvestRefer(address referAddress, uint256 amount) internal {
        ...
        for (uint256 i = 0; i < 6; i++) {
            if (uplineUserAddress != address(0) && uplineUserAddress != firstAddress) {
                if (currentDistributionLevel < 7) {
                    //NOTE: cache to INT in order to avoid revert in line 1180
                    int rew = int(((amount *
                        refRewardLevels[currentDistributionLevel]) / 1000));
                    int max = int(user_data[uplineUserAddress].maxPayout);
                    int claimed = int(user_data[uplineUserAddress].claimed);
    ...
    uint256 toClaim;
        ...
        //Changed for Testing
        require(
           ( block.timestamp - lastClaim ) >= 1 days,
 ...
 if (interest + claimed > maximumInterest) {
    ...
    function doubleUp() external nonReentrant {
        ...
        require(
            ...
            "Allowance should be double the intial amount."
        );
        ...
        //NOTE: cache to INT in order to avoid revert in line 1394
        ...
        int available = int(availableRewards(oldRef));
            ...
            1000);
        //NOTE: FIX: set `reinvestRefer` out side of clauses. Issue: does not update any upstream referrer if the base referrer exceeded `maxPayout`
        ...
        //NOTE: fixed wrong parameter issue, now using parameters from oldRef instead of the callers
        }else if( _max - (_claimed + available )>0 && oldRef != firstAddress){
                        int gib = _max   - (_claimed+ available) ; //NOTE: this can't be zero/negative due to the else if check
                        if(gib > _refRew){
                            gib= _refRew;
                        }
